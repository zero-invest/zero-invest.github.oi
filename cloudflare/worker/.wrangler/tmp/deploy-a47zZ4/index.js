...
var fundCatalog_default = [
  ...
  { code: "501227", detailMode: "summary", priority: 65, pageCategory: "domestic-lof", estimateMode: "market" }
];
...
async function syncSingleFund(code, config, fundQuotesMap, usQuotesMap, fxRate) {
  ...
  return {
    ...
    marketPrice,
    previousClose,
    marketDate: new Date(Date.now() + 8 * 60 * 60 * 1e3).toISOString().slice(0, 10),
    marketTime: new Date(Date.now() + 8 * 60 * 60 * 1e3).toISOString().slice(11, 19),
    premiumRate,
    proxyBasketName: config.benchmark,
    proxyQuotes,
    proxyQuoteDate: new Date(Date.now() + 8 * 60 * 60 * 1e3).toISOString().slice(0, 10),
    proxyQuoteTime: new Date(Date.now() + 8 * 60 * 60 * 1e3).toISOString().slice(11, 19),
      ...
      (async () => {
        try {
          ...
          if (result.ok) {
            ...
            console.error("[Scheduled] Self-fetch failed, falling back to source sync");
            const fallback = await syncRuntimeFromSource(env.RUNTIME_DB, env, { force: false });
            console.log("[Scheduled] Fallback result:", fallback.ok ? `synced ${fallback.fundCount} funds` : fallback.error);
          }
        } catch (error) {
